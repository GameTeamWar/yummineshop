...
export default function AddAddressModal({
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/50 z-50" onClick={() => setShowAddAddressModal(false)}></div>
      <div className={`fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-200 rounded-lg shadow-2xl border z-50 transition-all duration-300 ${darkMode ? 'bg-gray-800 border-neutral-700' : 'bg-white border-neutral-200'}`}>
        <div className={`flex items-center justify-between p-4 border-b ${darkMode ? 'border-neutral-700' : 'border-neutral-200'}`}>
          <h2 className={`text-lg font-semibold ${darkMode ? 'text-white' : 'text-neutral-950'}`}>Yeni Adres Ekle</h2>
          <button onClick={() => setShowAddAddressModal(false)} className={`p-2 rounded-lg transition-colors duration-300 ${darkMode ? 'hover:bg-neutral-700' : 'hover:bg-neutral-100'}`}>
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              onClick={async () => {
                if (newAddressName.trim() && addressCity.trim() && addressDistrict.trim() && user) {
                  try {
                    ...
                    const shortNeighborhood = newAddress.neighborhood && newAddress.neighborhood.length > 15
                      ? newAddress.neighborhood.substring(0, 15) + '...'
                      : newAddress.neighborhood || '';
                    setSelectedAddress(`${newAddress.name} (${shortNeighborhood})`);
