...
export default function Header({
  ...
  return (
    <>
        ...
        <div className="w-full mx-auto px-4 py-3">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              {/* Address Selection Button - Only for logged in users */}
              {user && (
              ...
              {user && (
                <button
                  ...
                  className={`p-2 rounded-lg transition-colors duration-300 relative ${darkMode ? 'bg-neutral-800 hover:bg-neutral-700' : 'bg-neutral-100 hover:bg-neutral-200'}`}
                >
              ...
              {user ? (
                <div className="relative user-dropdown">
                  <button onClick={() => setUserDropdownOpen(!userDropdownOpen)} className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-colors duration-300 ${darkMode ? 'bg-neutral-800 hover:bg-neutral-700' : 'bg-neutral-100 hover:bg-neutral-200'}`}>
                    <User className="w-4 h-4" />
                    <span className="text-sm font-medium hidden sm:inline">
                      Hoş Geldin, {userData?.firstName && userData?.lastName ? `${userData.firstName.charAt(0).toUpperCase() + userData.firstName.slice(1).toLowerCase()} ${userData.lastName.charAt(0).toUpperCase() + userData.lastName.slice(1).toLowerCase()}` : user?.displayName || user?.email?.split('@')[0] || 'Kullanıcı'}
                    </span>
                    <ChevronRight className={`w-4 h-4 transition-transform duration-300 ${userDropdownOpen ? 'rotate-90' : ''}`} />
                  </button>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => router.push('/auth/register?type=customer')} className={`px-4 py-2 rounded-lg font-semibold transition-colors duration-300 ${darkMode ? 'bg-neutral-800 hover:bg-neutral-700 text-white' : 'bg-neutral-100 hover:bg-neutral-200 text-neutral-950'}`}>
                    Kayıt Ol
                  </button>
                  <button onClick={() => router.push('/auth/login?type=customer')} className={`px-4 py-2 rounded-lg font-semibold transition-colors duration-300 ${darkMode ? 'bg-neutral-800 hover:bg-neutral-700 text-white' : 'bg-neutral-100 hover:bg-neutral-200 text-neutral-950'}`}>
                    Giriş Yap
                  </button>
                </div>
              )}
          ...
          {heroMode === 'shopping' && (
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Ürün, kategori, mağaza veya marka ara..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className={`w-full pl-10 pr-4 py-2.5 rounded-lg border transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-white text-sm sm:text-base ${darkMode ? 'bg-neutral-800 border-neutral-700 text-white placeholder-neutral-500 focus:bg-neutral-800' : 'bg-neutral-100 border-neutral-300 text-neutral-950 placeholder-neutral-500 focus:bg-white'}`}
              />
            </div>
          )}
        </div>
      ...
      {/* Cart Sidebar */}
      <div className={`fixed top-36 right-0 h-[calc(100vh-4rem)] w-80 z-50 transform transition-transform duration-300 ease-in-out cart-sidebar ${cartSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className={`h-full shadow-2xl ${darkMode ? 'bg-gray-800 border-l border-neutral-700' : 'bg-white border-l border-neutral-200'}`}>
          {/* Sidebar Header */}
