        ...
        <div className={`h-full flex flex-col shadow-2xl ${darkMode ? 'bg-gray-900 border-l border-neutral-700' : 'bg-white border-l border-neutral-600'}`}>
          ...
          <div className={`flex items-center justify-between p-4 border-b border-t ${darkMode ? 'border-neutral-700' : 'border-neutral-200'}`}>
            ...
            <div className="flex items-center gap-2">
              {cart.length > 0 && (
                <button onClick={() => { if (clearCart) clearCart(); setCart && setCart([]); setCartTotal && setCartTotal(0); }} className={`p-2 rounded-lg transition-colors duration-300 ${darkMode ? 'hover:bg-neutral-700 text-red-400 hover:text-red-300' : 'hover:bg-neutral-100 text-red-500 hover:text-red-600'}`}>
